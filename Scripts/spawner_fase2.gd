...
@export var enemy_definitions: Array[Dictionary] = [
	{
		"type": "ground",
		"scene": preload("res://Scenes/slime_azul.tscn")
	},
	{
		"type": "ground",
		"scene": preload("res://Scenes/TartatugaDeGrama.tscn")
	},
	{
		"type": "air",
		"scene": preload("res://Scenes/pernilongo.tscn")
	}
]
...
func spawn_enemy(wave: int):
	...
	for def in enemy_definitions:
		...
		if path == "res://Scenes/TartatugaDeGrama.tscn":
			if wave < 11:
				continue
			if randf() < 0.8:
				continue
...
func spawn_boss():
	var boss_scene = preload("res://Scenes/BossMosquitao.tscn")
